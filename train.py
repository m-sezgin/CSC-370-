...
from utilities.goturndataloader import GoturnDataloader
from utilities.vis_utils import Visualizer
from model.network import GoturnNetwork
from utilities.BoundingBox import BoundingBox
from utilities.draw_util import draw
from training.caffeSGD import CaffeSGD
...
class GoturnTrain(LightningModule):
    ...
    def train_dataloader(self):
        ...
        mean_file = None
        manager = Manager()
        objGoturn = GoturnDataloader(alov_path,
                                     mean_file=mean_file,
...
def get_args():
    ...
    ap.add_argument('--alov_path', type=str, 
                    default='/Users/michelesezgin/Desktop/Classes/CSC-370/final-project/GOTURN/my-goturn/data',
    ...
    ap.add_argument('--pretrained_model',
                    default='/content/gdrive/MyDrive/my-goturn/training/caffenet_weights.npy',
...
def main(hparams):
    ...
    ckpt_cb = ModelCheckpoint(dirpath=hparams.save_path, save_top_k=-1,
                              save_weights_only=False)
    trainer = Trainer(default_root_dir=hparams.save_path,
                      gpus=None, min_epochs=hparams.epochs,
                      accumulate_grad_batches=1,
                      # resume_from_checkpoint=ckpt_resume_path,
                      callbacks=[ckpt_cb], profiler=None)
    trainer.fit(model)
