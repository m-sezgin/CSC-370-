...
class AlovDataset:
    ...
    def __getitem__(self, idx):
        ...
        image_prev = load(prev_imgpath)
        image_prev = np.asarray(image_prev, dtype=np.uint8)
        image_curr = load(curr_imgpath)
...
if __name__ == "__main__":
    alovDir = '/content/gdrive/MyDrive/my-goturn/data'
    img_dir = Path(alovDir).joinpath('alov_frames_mini')
    ann_dir = Path(alovDir).joinpath('alov_annotations_mini')
    alovD = AlovDataset(str(img_dir), str(ann_dir), isTrain=False,
                        dbg=False)

    sample_gen = sample_generator(5, 15, -0.4, 0.4, dbg=False)
