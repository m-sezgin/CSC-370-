...
from .alov_alt_data_loader import AlovDataset
from .sampler import sample_generator
from .image_io import resize


class GoturnDataloader(Dataset):
    """Docstring for goturnDataloader. """

    def __init__(self, alov_path, mean_file=None, isTrain=True,
        ...
        # Alov
        img_dir = Path(alov_path).joinpath('alov_frames_micro')
        ann_dir = Path(alov_path).joinpath('alov_annotations_micro')
        ...
        self._minDataLen = len(self._alovD)
        self._maxDataLen = len(self._alovD)
        self._batchSize = 50
    ...
    def collate(self, batch):
        ''' Custom data collation for alov
        @batch: batch of data
        '''
        self._images = []
        self._targets = []
...
if __name__ == "__main__":
    alov_path = '/content/gdrive/MyDrive/my-goturn/data'
    manager = Manager()
    ...
    for i, *data in tqdm(enumerate(dataloader), desc='Loading alov', total=len(objGoturn) / 3, unit='files'):
        pass
